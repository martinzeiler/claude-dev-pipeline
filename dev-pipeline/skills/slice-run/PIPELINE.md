...
1. **Červená napřed:** pro každé akceptační kritérium s testovatelným povrchem napiš nejdřív test (vitest, pokud projekt harness má) a ověř, že selhává ze správného důvodu (chybějící funkčnost, ne syntax error). U kritérií pokrytých jen E2E ověř červenou přes agenta `dev-pipeline:e2e-verifier` (scénář z `docs/e2e/` proti běžící appce PŘED implementací), pokud to dává smysl (u zcela nové obrazovky netřeba).
   ...
   - **Mutaci vracej opačnou editací. NIKDY žádným git příkazem, který sahá na pracovní strom** — `checkout`, `restore`, `stash`, `clean`, `reset --hard`. U souboru, který ještě není v gitu, `git checkout` nevrátí mutaci, ale **zahodí celý soubor** (jednou to sebralo 849 řádků práce ze tří dávek) a `git stash` odklidí i rozpracovanou práci souběžných lane. Když opravdu potřebuješ porovnat proti `HEAD`, vytáhni si obsah **vedle**: `git show HEAD:<cesta> > /tmp/orig.ts`, nebo si založ `git worktree`.
   - **Mutační ověření je povinné u každého záporného akceptačního kritéria**, ne jen náhrada červené u nových modulů: napiš stráž → **zkus ji obejít jinou konstrukcí, než na kterou míří** → když projde, stráž není hotová. Doloženo pěti selháními napříč dvěma řezy, pokaždé zeleně: guard hlídal řetězcové literály (obešel ho bare klíč objektu), pak jméno metody (obešel ho `for` cyklus), pak syntaktický strom (obešla ho jediná lokální proměnná — 12 z 12 pokusů prošlo); zavřela to až typová analýza. Do souhrnu patří tabulka **mutace → výsledek**.
   - **Stráž nad textovou inkluzí není důkaz chování.** Test, který jen hledá jméno tabulky nebo funkce v souboru, zůstane zelený i po odstranění toho, co má hlídat. Když má guard hlídat, že se něco *děje*, musí to spustit.
2. Implementuj podle PRD. Řiď se CLAUDE.md cílového projektu (konvence, pasti, helpers) — má přednost před obecnými zvyky. Soubory >500 LOC edituj přes Serena symbol tools, pokud je projekt má nakonfigurované; hledání symbolů (definice, volající, přehled souboru) přes ně dělej bez ohledu na velikost souboru — vrátí symbol místo celého souboru.
...
Tři pravidla, bez kterých dělení tiše ztrácí práci:

- **Každé akceptační kritérium namapuj na právě jednu lane** a kritéria, která hranice překračují (typicky záporná a rozsahové pojistky „řez nestaví X"), dej výslovně **dodělávkové lane** běžící po ostatních. Je to mechanická kontrola: seznam AK z PRD proti seznamu lane. Doloženo: pojistka sahala přes `packages/db`, `apps/api` i `apps/admin`, takže ji nenapsal nikdo — každý agent legitimně zůstal ve svém rozsahu a odhalilo to až review.
- **Sdílená vlna vydává i rozhodnutí, ne jen strukturu.** Vlna 1 v ostrém běhu vydala tvar sekce a stavy buněk, a čtyři z pěti rozchodů mezi souběžnými plochami padly přesně do toho, co nevydala: formátování procent, věta o řádku, měna prázdné identity, chování hlavičky u prázdného stavu. Obě plochy si odpověděly rozumně a jinak. Buď vydej i „jak se to píše", nebo výslovně vyjmenuj rozhodnutí, která necháváš na plochách, ať je scelovací vlna hledá cíleně.
- **Brána je per balíček, celek řeší scelovací vlna.** Root `typecheck`/`test` může být červený na rozpracovaných souborech souběžné lane — do zadání každé lane proto patří věta „root brána může být červená na cizích souborech, ověř svůj balíček". A když lane narazí na červenou **mimo svůj rozsah**, jen ji ohlásí a nechá být; diagnóza patří tomu, kdo to bude opravovat. (Dvakrát se stalo, že dvě lane nezávisle rozebíraly týž flaky test.) Když orchestrátor zná stav brány předem, předá ho v zadání jako fakt.
...
**Druhé a další kolo se zužuje na opravnou várku**, ne na celý diff: „prověř výhradně tohle, co prošlo předchozím kolem znovu nekontroluj". Zadání zároveň vyžaduje, aby každý nález nesl `BLOKUJE NASAZENÍ` nebo `FOLLOW-UP` — agent `code-review` to má v kontraktu, u general-purpose náhrady to musí být v promptu.

**Kdy tu smyčku utnout** (bez tohohle kritéria nemá konec — v jednom ostrém řezu si každé opravné kolo vyžádalo další, kolo 1 → 5 zásahů, kolo 2 → 6): **rozšíření, které jde ověřit deterministicky, uzavře orchestrátor sám; rozšíření, které mění rozhodovací logiku, si žádá další kolo.** Deterministicky = mutací, typem, testem — například nový plošný guard ověříš tak, že do souboru mimo allowlist vložíš zakázaný vzor, guard zčervená a ty to vrátíš; stojí to dvě minuty. Naproti tomu sjednocení rozhodování o důvodech přes tři metriky do dalšího kola patří — a v tom ostrém běhu tam našlo dvě blokující vady. Rozhodnutí zapiš do journalu.

**Paralelní fix agenti dostávají disjunktní množiny souborů — a v souběhu commituje výhradně orchestrátor.** Hranice se dělá po souborech, ne po nálezech; nikdy „ty vezmi tyhle nálezy, ty tamty", když se nálezy potkávají nad týmž souborem. Disjunktní soubory ale nestačí, protože **pre-commit brána je sdílená nad celým repem**: agent, který chtěl commitnout bezpečnostní opravu, měl své soubory zelené a `typecheck` mu padal na rozpracovaném souboru cizí větve — vyřešil to `core.hooksPath=/dev/null`, tedy přesně tím, co projekt zakazuje. Proto: (1) commituje orchestrátor, až doběhnou všechny větve; (2) když větev narazí na bezpečnostní nález, který doktrína velí commitnout hned, **nahlásí ho a commit nechá orchestrátorovi**; (3) v zadání souběžné větve musí stát, že **root brána může být červená na cizích souborech — agent verifikuje svůj balíček**.
...
**Známá třída vady, kterou hledání nenajde: NUL byty ve zdrojáku.** Agenti si je zavlečou skládaným klíčem nebo escape sekvencí ve `Write` a `ripgrep` takový soubor **tiše přeskočí jako binární** — stane se neviditelným pro půlku nástrojů v repu včetně těch, kterými agenti hledají. `grep -P '\x00'` ho nenajde; ověřuje se binárně (`readFileSync(f).some(b => b === 0)`, nad dvěma tisíci soubory to trvá jednotky milisekund). V jednom řezu se to stalo dvakrát, podruhé přímo při opravě toho prvního.
...
**Zadání verifikátora u plochy s nevratným vnějším účinkem** (e-mail, webhook, platba, zápis do cizího systému) musí obsahovat **pojmenovanou pojistku a její ověření**, ne jen rozpočet: nejdřív zapnout (např. globální přesměrování pošty), ověřit ji na prvním odeslání a teprve pak pokračovat. K tomu **rozpočet nevratných akcí** a **kontrolní součet stavu před a po** (u ostrých dat: kolik záznamů, kolik publikovaných, bajtová shoda obsahu). Report se dá smazat, odeslaná zpráva ne.
...
1. PRD frontmatter: `status: done` + `commit: <hash posledního commitu řezu>` (strojová kotva pro rollback dalších řezů). Smaž `docs/.deploy-unlocked`.
   - **Srovnej PRD a E2E s tím, co se skutečně postavilo.** Vědomá odchylka od PRD je legitimní a častá (implementace správně zahodí funkci, která nemá volajícího; prvek se přesune jinam), ale **dokument nesmí tvrdit něco jiného než kód** — jinak příští `plan-check` nebo validátor označí hotový řez za nedodělaný a E2E krok hlásí rozpor, který není vadou. Uprav dotčené pasáže PRD i scénářů a odchylku zapiš do journalu. Stalo se doslova: PRD i scénář dál popisovaly funkci, kterou opravný agent správně smazal, a po nasazení tvrdila tři místa tři různé věci.
2. Append do `docs/journal.md`: datum, řez NN, co je hotové, odchylky od vize/osnovy + proč, změněná rozhodnutí, počet pokusů, výsledek E2E. **Zapisuj Bash heredocem (`cat >> … <<'EOF'`), nikdy Editem** — Edit do velkého souboru platí kotvu v `old_string` i `new_string`, takže jeden zápis do journalu stojí i pár kilobajtů kontextu navíc oproti tomu, co se doopravdy přidává.
3. Nápady a resty mimo scope → append `docs/follow-ups.md` (jedna odrážka = jedna položka, s kontextem proč) — opět heredocem, ne Editem. Pokud řez mimochodem vyřešil existující follow-up, položku nemaž, ale přeškrtni (`~~text~~`) a připiš `VYŘEŠENO <datum>: <čím, commit>` — odškrtávat smí jen ten, kdo si vyřešení ověřil proti kódu/aplikaci, ne podle dojmu.
4. Přepiš `docs/handoff.md`: branch, poslední done řez, stav (co funguje), co je logicky další, klíčové pasti/poznatky z tohoto řezu (**max ~30 řádků / 2 kB** — čte to čerstvý kontext a přepisuje se celý Writem, takže každý nadbytečný odstavec platíš při každém uzavření řezu znovu; naměřeno 7 kB na přepis, čtrnáctkrát za běh). Neduplikuj obsah PRD/journalu — odkazuj cestou. Do handoffu ani journalu nikdy nepatří secrets (klíče, hesla, tokeny).
...
6. **Formátování stavových souborů.** Fáze, která zapsala do `docs/*.md` (journal, handoff, follow-ups, vize-spory), na ně **rovnou pustí formátovač projektu** (`prettier --write <soubory>` nebo co projekt používá). **Když projekt žádný formátovač nemá** (není v `package.json` ani na PATH), krok odpadá — zapiš to jednou do journalu a dál se k tomu nevracej; nehledej náhradu a nespouštěj nic globálního. Zapisuje se heredocem a `format:check` pak spadne někomu, kdo ty soubory nezaložil ani needitoval — a musí sáhnout na cizí stavový soubor, aby jeho fáze mohla skončit zeleně.
