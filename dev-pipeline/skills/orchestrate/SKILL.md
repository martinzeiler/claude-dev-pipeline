...
3. **Archivace předchozí vize:** pokud `docs/prd/` obsahuje PRD patřící jiné vizi (frontmatter `vize:` ≠ aktuální), přesuň `docs/prd/*.md`, `docs/e2e/*.md` a `docs/journal.md` do `docs/archive/<slug-předchozí-vize>/` (git mv, samostatný commit `archiv: <slug>`) a smaž stale markery `docs/.vize-done` + `docs/.review-passed` (jinak by PRD agent novou vizi rovnou prohlásil za hotovou). `docs/follow-ups.md` a `docs/vize/` se NIKDY nearchivují — follow-ups je kontinuální backlog napříč vizemi. Pak založ chybějící stavové soubory (`docs/journal.md`, `docs/handoff.md`, `docs/follow-ups.md`, `docs/prd/`, `docs/e2e/`), přepiš `docs/handoff.md` na čistý start nové vize a `touch docs/.orchestrator-run`.
