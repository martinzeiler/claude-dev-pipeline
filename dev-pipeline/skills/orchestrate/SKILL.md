...
2. Pokud je vize necommitnutá, commitni ji jako první akt (samostatný commit `vize: <slug>`). Pak ověř čistý working tree. Vytvoř/checkoutuj branch `vize/<slug>`. Doplň markery (`docs/.orchestrator-run`, `docs/.deploy-unlocked`, `docs/.vize-done`, `docs/.review-passed`) do `.gitignore`, pokud chybí.
3. **Archivace předchozí vize:** pokud `docs/prd/` obsahuje PRD patřící jiné vizi (frontmatter `vize:` ≠ aktuální), přesuň `docs/prd/*.md`, `docs/e2e/*.md`, `docs/journal.md` a `docs/zaverecna-zprava.md` do `docs/archive/<slug-předchozí-vize>/` (git mv, samostatný commit `archiv: <slug>`) a smaž stale markery `docs/.vize-done` + `docs/.review-passed` (jinak by PRD agent novou vizi rovnou prohlásil za hotovou). Zároveň **kompaktuj follow-ups**: přeškrtnuté položky (vyřešené/převzaté/zamítnuté) přesuň z `docs/follow-ups.md` do `docs/archive/<slug-předchozí-vize>/follow-ups-uzavrene.md` — živý soubor drží jen otevřené položky. Samotný `docs/follow-ups.md` a `docs/vize/` se NIKDY nearchivují celé — follow-ups je kontinuální backlog napříč vizemi. Pak založ chybějící stavové soubory (`docs/journal.md`, `docs/handoff.md`, `docs/follow-ups.md`, `docs/prd/`, `docs/e2e/`), přepiš `docs/handoff.md` na čistý start nové vize a `touch docs/.orchestrator-run`.
...
5. Zapiš start běhu do journalu.
...
4. **Lehké review** (PIPELINE fáze 4): spusť subagenta `dev-pipeline:code-review` (effort `medium`; diff si posbírá sám). NIKDY nedělej review v této session a **neinvokuj skill `code-review`** — má `disable-model-invocation: true`, žádný model ho přes Skill tool nespustí. Nálezy → fix agent (předej mu seznam nálezů, ne diff). Poté krátký verify agent: typecheck + testy.
