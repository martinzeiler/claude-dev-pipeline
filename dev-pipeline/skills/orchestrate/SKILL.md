...
Kanonická definice fází řezu: `PIPELINE.md` ve vedlejším skillu `slice-run` (cestu odvoď z umístění tohoto SKILL.md: `../slice-run/PIPELINE.md`; pokud ji neznáš, najdi Globem `**/dev-pipeline/skills/slice-run/PIPELINE.md` v `~/.claude/plugins/`, `~/claude-dev-pipeline/` a `~/`). Přečti si ho na začátku — uživatel může skilly za běhu upravovat a injektovaný text téhle invokace může být stale kopie z plugin cache. Při rozporu má vždy přednost verze na disku; že jedeš podle disku, poznamenej do handoffu. Subagentům vždy předávej **absolutní cestu** k PIPELINE.md.

**Na začátku každého dalšího řezu ho nečti znovu celý — porovnej hash.** Při prvním čtení si ulož `shasum -a 256 <cesta k PIPELINE.md>` do handoffu; na začátku dalšího řezu spusť týž příkaz (jednořádkový výstup) a `Read` pusť **jen když se hash liší**. Soubor má přes 37 kB, takže každé zbytečné přečtení stojí ~9k tokenů tvého kontextu, a mezi řezy se mění výjimečně. Po compactu, kdy hash v paměti nemáš, ho přečti znovu a hash si ulož.
...
2. **prd-check** (`dev-pipeline:prd-check`, PIPELINE fáze 2): předej cesty k PRD, vizi, severce a **cestu pro report** (`docs/reviews/rez-NN-prd-check-kolo-M.md`). Vrátí verdikt, počet blokujících a osy s nálezem; samotné nálezy nevrací a plný report **nečteš** — jeho cestu předáš PRD-fix agentovi, který si je přečte a zapracuje. Opakovací kolo dle PIPELINE.md fáze 2 (vč. jediné povolené výjimky pro přeskok).
...
4. **Lehké review** (PIPELINE fáze 4): spusť `dev-pipeline:code-review` s `rozsah: pracovní-strom` a cestou pro report `docs/reviews/rez-NN-code-review-kolo-M.md` (diff si posbírá sám). NIKDY nedělej review v této session a **neinvokuj skill `code-review`** — má `disable-model-invocation: true`, žádný model ho přes Skill tool nespustí. Review ti vrátí **rozdělení nálezů do disjunktních balíčků po souborech** — podle nich spusť fix agenty (jeden na balíček; každý dostane **cestu k reportu a čísla svých nálezů, nikdy diff a nikdy obsah reportu**). Balíček označený `[security]` znamená samostatný commit. Plný report ani jednotlivé nálezy sám nečti — rozděleno je za tebe právě proto, abys nemusel. Pak brána: **samotný typecheck si pusť sám** (při úspěchu je to dvouřádkový výstup, agent by stál víc, než ušetří), **plnou testovou suitu pošli `dev-pipeline:verify`** — ta při pádu vrací stovky řádků, které ve tvém kontextu nemají co dělat.
...
6. **e2e-verifier** (`dev-pipeline:e2e-verifier`, PIPELINE fáze 6): předej mu i **cestu pro report** (`docs/reviews/rez-NN-e2e-kolo-M.md`) — tabulka kritérií jde tam, tobě se vrátí jen strojový řádek, FAIL kritéria a závažné nálezy mimo AK. FAIL → fix agent (dostane cestu k reportu) → deploy agent → e2e znovu; počítej pokusy dle failure policy PIPELINE.md. **Po 2. funkčním neúspěchu nespouštěj třetí stejný pokus — spusť `dev-pipeline:diagnose`** (nepočítá se do pokusů) a teprve s jeho doloženou příčinou jde třetí pokus.
...
**Uvnitř řezu piš krátce a o postupu, ne o nálezech.** Uživatel chce vědět, kde běh je („implementace hotová, běží review a souběžně PRD dalšího řezu"), ne co našlo které kolo. Nálezy review, prd-checku ani E2E **nepřevyprávěj** — na to je report, který si otevře jen ten, kdo je opravuje; do zprávy patří nanejvýš počet a to, jestli něco blokuje nasazení. Uzavření řezu je jediné místo, kde se píše víc: co je hotové, co šlo do follow-ups a co potřebuje jeho rozhodnutí. Doloženo přímo od uživatele: „Řez 01 hotový a uzavřený, to se mi líbí — ale ostatní bych vidět nemusel."
...
- **Do zadání běžce patří doslova věta „mezi kroky řetězu nečekej pollingem — spusť subagenta, ukonči tah a navaž notifikací".** Zákaz pollingu máš ve svém skillu, ale běžec je general-purpose agent, který tenhle soubor nikdy neuvidí — a bez té věty si vyrobí bashovou `sleep` smyčku nad výstupním souborem podagenta. Naměřeno dvakrát po sobě: běžec řezu 02 tak prostál **3 hodiny** poté, co jeho PRD agent dopsal (21 z 33 bash volání byl `sleep`), a jeho nástupce v řezu 03 začal stejně; před uživatelem to navíc vypadá jako zaseknutá pipeline. Nejde o nedostatek nástrojů — jeden z nich si `Monitor` dokonce načetl a pak ho nepoužil, protože si myslel, že ukončit tah znamená přerušit řetěz.
- **Fakta o stavu řezu N předej souborem, ne v promptu.** Zadání běžce je jediné místo, kde ti strop 2 000 znaků pravidelně praská (naměřeno 3 849 a 3 945 znaků), protože do něj vypisuješ, co řez N postavil. Napiš ten stav do `docs/reviews/rez-NN-stav-pro-blok.md` a pošli cestu — je to táž informace za desetinu kontextu, a běžec ji navíc předá dál PRD agentovi, aniž by ji převyprávěl potřetí.
