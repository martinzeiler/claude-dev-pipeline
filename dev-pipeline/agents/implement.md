...
3. **Velké soubory přes symbol tools.** Soubor nad ~500 LOC edituj přes Serena (`find_symbol` / `replace_symbol_body` / `find_referencing_symbols`, cross-file rename přes `rename_symbol`), pokud je projekt má nakonfigurované. Read celého souboru + Edit je u nich zbytečně drahé a náchylné na kolize.
