...
2. **`/dev-pipeline:orchestrate`** — v nové session; na dlouhý běh „spusť a odejdi" v tmuxu s limit-watcherem (viz Usage limity). Co proběhne samo:
   ...
   - **Souběh** (od 0.4.0, ať se využije pětihodinové okno): PRD dalšího řezu se píše vedle deploye a E2E toho současného; implementace řezu sahajícího na víc balíčků se dělí (nejdřív sdílené typy, pak backend a frontend souběžně); fix agenti běží paralelně nad disjunktními soubory; obě bezpečnostní metodiky v kolečku běží najednou. **Dva řezy najednou nikdy** — sdílená produkce.
